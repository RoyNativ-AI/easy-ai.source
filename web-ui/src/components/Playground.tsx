...
const Playground: React.FC = () => {
  ...
  const [collapsedProviders, setCollapsedProviders] = useState<Set<string>>(new Set(['Anthropic', 'OpenRouter']))
  const [showModelFilters] = useState(false)
  ...
  const fetchModels = async () => {
    try {
      ...
      if (models.length > 0) {
        // Try to select the latest GPT-4 model, fallback to first model
        const preferredModel = models.find(m => m.id === 'chatgpt-4o-latest') || 
                              models.find(m => m.id === 'gpt-4o-latest') ||
                              models.find(m => m.id.includes('gpt-4o') && m.id.includes('latest')) ||
                              models.find(m => m.id.includes('gpt-4')) ||
                              models[0]
        setSelectedModels([preferredModel.id])
      }
    } catch (error) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      try {
        ...
        newFileContents[file.name] = text
      } catch (error) {
    ...
    // Auto-generate prompt with file contents
    if (newFiles.length > 0) {
      const filePrompt = newFiles.map(file => 
        `File: ${file.name}\nContent:\n${newFileContents[file.name] || 'Loading...'}\n\n`
      ).join('')
      
      setPrompt(filePrompt + (prompt ? `\nAdditional instructions:\n${prompt}` : ''))
    }
  }
  ...
  const removeFile = (fileName: string) => {
    ...
    // Regenerate prompt without the removed file
    if (updatedFiles.length > 0) {
      const filePrompt = updatedFiles.map(file => 
        `File: ${file.name}\nContent:\n${updatedContents[file.name] || 'Loading...'}\n\n`
      ).join('')
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    backgroundColor: inputMode === 'text' ? '#5b61eb' : '#f9fafb',
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    backgroundColor: inputMode === 'file' ? '#5b61eb' : '#f9fafb',
                    color: inputMode === 'file' ? '#ffffff' : '#374151',
                    border: '1px solid #d1d5db',
                    borderRadius: '6px',
                    fontSize: '0.875rem',
                    fontWeight: '500',
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <div style={{
                    ...
                    <input
                      type="file"
                      multiple
                      accept=".txt,.md,.js,.ts,.jsx,.tsx,.py,.json,.csv,.xml,.html,.css,.sql,.log"
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        Supports: TXT, MD, JS, TS, PY, JSON, CSV, XML, HTML, CSS, SQL, LOG
                      </div>
                    </label>
                  </div>
